...
We first use classification trees to analyze the =Carseats= dataset.
In these data, =Sales= is a continuous variable, so we begin by recoding it as a binary variable.
We then use the =tree()= function to fit a classification tree.
